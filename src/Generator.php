 ...
 * @property string $stickyDataFile
 */
abstract class Generator extends Model
{
     ...
     * Please refer to [[\anggagewor\giing\generators\controller\Generator::generate()]] as an example
    ...
    {
        return Yii::$app->getRuntimePath() . '/gii-' . Yii::getVersion() . '/' . str_replace('\\', '-', get_class($this)
            ) . '.json';
    }
    ...
    public function save( $files, $answers, &$results )
    {
        ...
        foreach ( $files as $file ) {
            ...
            if ( isset($answers[ $file->id ]) && !empty($answers[ $file->id ])
                 && $file->operation !== CodeFile::OP_SKIP ) {
                $error = $file->save();
                if ( is_string($error) ) {
                    $hasError = true;
                    $lines[]  = "generating $relativePath\n<span class=\"error\">$error</span>";
                } else {
                    $lines[] = $file->operation
        ...
        try {
            if ( class_exists($class) ) {
                if ( isset($params[ 'extends' ]) ) {
                    if ( ltrim($class, '\\') !== ltrim($params[ 'extends' ], '\\')
                         && !is_subclass_of($class, $params[ 'extends' ]) ) {
