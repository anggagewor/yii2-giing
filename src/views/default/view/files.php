...
use anggagewor\giing\CodeFile;
use yii\helpers\Html;

/* @var $this \yii\web\View */
/* @var $generator \anggagewor\giing\Generator */
/* @var $files CodeFile[] */
/* @var $answers array */
    ...
    <table class="table table-bordered table-striped table-sm">
        ...
        <?php foreach ( $files as $file ): ?>
            ...
            <tr class="<?= "$file->operation $trClass" ?>">
                <td class="file">
                    <?= Html::a(Html::encode($file->getRelativePath()), [ 'preview', 'id' => $id, 'file' => $file->id ],
                        [ 'class' => 'preview-code', 'data-title' => $file->getRelativePath() ]
                    ) ?>
                    <?php if ( $file->operation === CodeFile::OP_OVERWRITE ): ?>
                        <?= Html::a('diff', [ 'diff', 'id' => $id, 'file' => $file->id ],
                            [ 'class' => 'diff-code badge badge-warning', 'data-title' => $file->getRelativePath() ]
                        ) ?>
                    <?php endif; ?>
                </td>
                <td class="action">
                ...
                <?php if ( $fileChangeExists ): ?>
                    <td class="check">
                        ...
                        if ( $file->operation === CodeFile::OP_SKIP ) {
                            ...
                            echo Html::checkBox("answers[{$file->id}]",
                                isset($answers) ? isset($answers[ $file->id ]) : ( $file->operation
                                                                                   === CodeFile::OP_CREATE )
                            );
                        }
                        ?>
